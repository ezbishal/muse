...
// const STREAMS_REFRESH_DELAY = 10 * 1000;
const ASPECT_MULTIPLIER = 20;
...
export default function StreamView({
  ...
  async function refreshStreams() {
    ...
    if (playerRef.current && !ytRef.current) {
      ytRef.current = YoutubePlayer(playerRef.current, {
        width: 16 * ASPECT_MULTIPLIER,
        height: 9 * ASPECT_MULTIPLIER,
      });

      ytRef.current.loadVideoById(activeStream?.extractedId ?? "");

      ytRef.current.on("ready", () => console.log("Player is ready"));
      ytRef.current.on("stateChange", (e) => {
        if (e.data === 0) nextStream();
      });
    }
  }
  ...
  const nextStream = async () => {
    ...
    ytRef.current?.loadVideoById(response.data.stream.extractedId);
